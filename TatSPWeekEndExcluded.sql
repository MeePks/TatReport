...
Select	
datediff(HOUR,
	Case when DateName(WeekDay,coalesce(i.FileLandedDate, ir.FileLandedDate,ifs.RAWFileCreateDate,isp.FileLandedDate ))='Saturday'
			Then cast(Cast(DATEADD(Day,2,coalesce(i.FileLandedDate, ir.FileLandedDate,ifs.RAWFileCreateDate,isp.FileLandedDate )) as date) as datetime)
		when DateName(WeekDay,coalesce(i.FileLandedDate, ir.FileLandedDate,ifs.RAWFileCreateDate,isp.FileLandedDate ))='Sunday'
			Then Cast(Cast(DATEADD(Day,1,coalesce(i.FileLandedDate, ir.FileLandedDate,ifs.RAWFileCreateDate,isp.FileLandedDate )) as date) as datetime)
 Else coalesce(i.FileLandedDate, ir.FileLandedDate,ifs.RAWFileCreateDate,isp.FileLandedDate ) END,
LoadEndDate) as TAT,
